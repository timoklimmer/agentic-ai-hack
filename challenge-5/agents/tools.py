...
class CosmosDBPlugin:
    ...
    def _get_cosmos_client(self):
        ...
        if not self.endpoint or not self.key:
            raise Exception(
                "Cosmos DB endpoint and key must be configured. Please set COSMOS_ENDPOINT and COSMOS_KEY environment variables.")

        try:
            from azure.cosmos import CosmosClient
            # return CosmosClient(self.endpoint, self.key)
            import azure.identity as aid
            return CosmosClient(self.endpoint, credential=aid.DefaultAzureCredential())
        except ImportError:
            raise ImportError(
    ...
    def get_document_by_claim_id(
        self,
        claim_id: Annotated[str,
                            "The claim_id to retrieve (not the partition key)"]
        ...
        try:
            ...
            if not items:
                ...
                available_ids = [item.get('claim_id', 'N/A')
    ...
    def get_document_by_id(
        ...
        partition_key: Annotated[str,
                                 "The partition key value (optional, will use cross-partition query if not provided)"] = None
    ) -> Annotated[str, "JSON document from Cosmos DB"]:
    ...
    def query_documents(
        self,
        sql_query: Annotated[str,
                             "SQL query to execute (e.g., 'SELECT * FROM c WHERE c.category = \"electronics\"')"]
    ...
    def list_recent_documents(
        self,
        limit: Annotated[int,
                         "Maximum number of documents to return (default: 10, max: 100)"] = 10
